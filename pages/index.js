...
export default function Home() {
  ...
  const handleSubmit = async (event) => {
    ...
    const categorizedAddresses = categorizeAddresses(addressList); // 
    const tokensData = await fetchTokenData(categorizedAddresses); // 
  ...
  return (
    <div className="d-flex vh-100 justify-content-center align-items-center bg-white">
      ...
      <form onSubmit={handleSubmit} className="text-center">
        ...
        />
        <button type="submit" className="btn btn-primary btn-lg">检查资格</button> {/* Optional: Use btn-lg to enlarge the button as well */}
      </form>
        <div className="mt-3">
        {Object.keys(tokens).length > 0 && (
          ...
          {Object.entries(tokens).map(([address, tokenAmount]) => (
             <div key={address} className="token-amount">{`${address}: ${tokenAmount}`}</div>
          ))}
          <Analytics />
        </div>
      </div>
